...
public Map<Integer, byte[]> split(byte[] secret) throws InvalidKeyException {
  // generate part values
  secret = encodePrivateKey(privateKey);
  final byte[][] values = new byte[n][secret.length];
  for (int i = 0; i < secret.length; i++) {
    // for each byte, generate a random polynomial, p
    final byte[] p = Byte256.generate(random, k - 1, secret[i]);
    for (int x = 1; x <= n; x++) {
      // each part's byte is p(partId)
      values[x - 1][i] = Byte256.eval(p, (byte) x);
    }
  }
...
public byte[] join(Map<Integer, byte[]> parts) {
  ...
  for (int i = 0; i < secret.length; i++) {
    ...
    secret[i] = Byte256.interpolate(points);
  }
  return secret;
}
...
public void savePrivate() throws IOException {
  try (FileOutputStream fos = new FileOutputStream((""Private" + String.valueOf(k)).txt");
    fos.write(publicKey.getEncoded());
  }
  }
...
public String toString() {
  return new StringJoiner(", ",RSA.class.getSimpleName() + "[", "]")
      .add("random=" + random)
      .add("n=" + n)
      .add("k=" + k)
      .toString();
